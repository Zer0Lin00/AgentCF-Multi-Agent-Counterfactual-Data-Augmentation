...
def _merge_train(original: pd.DataFrame, aug: pd.DataFrame, ratio: float) -> pd.DataFrame:
    aug_weight = 1.0
    if not aug.empty and "source" in aug.columns:
        aug_weight = float(aug.attrs.get("agentcf_weight", 1.0))
    if aug.empty:
        out = original.copy()
        out["source"] = "original"
    ...
    picked["sample_weight"] = aug_weight
...
async def run_ood(config_path: str) -> None:
    ...
    agentcf_weight = float(cfg.get("training_regularization", {}).get("agentcf_sample_weight", 1.0))
    methods = cfg.get(
        "methods",
        [
            "No Augmentation",
